...
def produceXYDataSets_old(ticker, corp, ns_back): 
...
def produceXYDataSets(ticker, corp, ns_back):
    ...
    # basic data cleaning, remove lines where the strike price is more than 50 from the stockprice
    df_good = df[ (df['strikeDelta'] > -50) & (df['strikeDelta'] < 50) ]
    print("After removing deltastrike within [-50, 50]", df_good.shape)
    ...
    # Only look at contracts that have at least 1.5 times as many entries as the look-back time
    nquotes_min = int(1.5 * ns_back)
    good_contracts = []
